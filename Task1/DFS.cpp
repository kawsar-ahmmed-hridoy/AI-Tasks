...
bool dfs(unordered_map<char,vc> &adj, unordered_set<char> &vis, unordered_map<char, char> &par, char cur, char dst) {
    vis.insert(cur);
    if (cur == dst) return 1;
    ...
    for (char child : adj[cur]) {
        if (!vis.count(child)) {
            par[child] = cur;
            if (dfs(adj, vis, par, child, dst)) return 1;
        }
    }
    return 0;
}
int main() {
    ...
    bool flag = dfs(adj, vis, par, src, dst);
    
    if(!flag)cout<<"Not reachable."<<endl;
    else{
        vc path;
        char xn = dst;
        while(xn!=src){
